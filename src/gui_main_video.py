...
class MicControlApp(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Button area - single button only
    ...
    def start_talking(self):
        ...
        if self.current_state == "ready":
            ...
            print("[GUI] 📢 Starting listening process")
            bridge.update_button.emit("🔄 Starting...", False)
            
            # Run STT trigger in executor
    ...
    def _check_and_start_stt(self):
        ...
        try:
            # Health checks moved to terminal logging only
            print(f"[GUI] 🎤 Starting STT process...")
            
            # Prerequisites OK, proceed with STT
            print(f"[GUI] ✅ Starting STT...")
            ...
            print(f"[GUI] ❌ Error in STT trigger: {e}")
            import traceback
            traceback.print_exc()
...
def redis_listener():
    ...
    # Health check on startup (terminal logging only)
    health_checker = ServiceHealthChecker()
    print("\n[Health] 🚀 Startup health check...")
    healthy, message = health_checker.check_all_services()
    if healthy:
...
def main():
    ...
    print("[GUI] 🎵 Health checks moved to terminal logging only")
